    ...
    <div className="flex w-full flex-col pb-[96px]">
      ...
      {visible.map((post) => (
        <Link
          key={post.slug}
          href={`/blog/${post.slug}`}
          className="flex w-full flex-col gap-[8px] border-t border-hairline p-[24px] transition-colors last:border-b hover:bg-ink/[0.03] lg:flex-row lg:items-center lg:justify-between lg:gap-[24px]"
        >
          <div className="flex min-w-0 items-center justify-between gap-[24px] lg:justify-start">
            ...
            <span className="hidden min-w-0 text-[16px] leading-normal text-ink lg:block">
              {post.title}
            </span>
            <span className="shrink-0 text-[10px] leading-[13px] text-ink-50 lg:hidden">
              {post.tag}
            </span>
          </div>
          <span className="text-[16px] leading-normal text-ink lg:hidden">{post.title}</span>
          <span className="hidden shrink-0 text-[10px] leading-[13px] text-ink-50 lg:block">
            {post.tag}
          </span>
        </Link>
      ))}
