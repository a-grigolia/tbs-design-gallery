...
export default async function OutdoorLivingPage() {
  ...
  const { docs: vendors } = await payload.find({
    ...
    where: {
      and: [
        { categories: { contains: 'outdoor-living' } },
        { active: { equals: true } },
    ...
    <div className="flex min-h-screen w-full flex-col items-center bg-canvas">
          ...
          <div className="flex w-full flex-col items-center px-gutter-sm py-[24px] lg:px-gutter">
            <div className="flex w-full max-w-[384px] flex-col gap-[16px]">
              ...
              <p className="text-[14px] leading-[18px] text-ink-50">
                TBS Design Gallery has chosen to work with two companies in line with our standards
                of excellence and our firm belief in the importance of biophilic living &mdash;
                otherwise known as &ldquo;design with the intention of connecting human beings with
                their natural surroundings.&rdquo; Those companies are Renson and Premier Outdoor
                Cabinetry.
              </p>
            </div>
          </div>
      ...
      <SectionBand>
        <BlueprintColumn className="pb-section">
          <VendorCta category="outdoor-living" />
          <TickRule label="All outdoor living" />
