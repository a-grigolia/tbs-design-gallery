...
export default async function ArchitecturalElementsFurniturePage() {
  const payload = await getPayload()
  const { docs: vendors } = await payload.find({
    collection: 'vendors',
    where: {
      and: [
        { categories: { contains: 'architectural-elements-furniture' } },
        { active: { equals: true } },
    ...
    <div className="flex min-h-screen w-full flex-col items-center bg-canvas">
          ...
          <div className="flex w-full flex-col items-center px-gutter-sm py-[24px] lg:px-gutter">
            <div className="flex w-full max-w-[384px] flex-col gap-[16px]">
              ...
              <p className="text-[14px] leading-[18px] text-ink-50">
                Discover the perfect blend of expert craftsmanship and innovative design with TBS
                Design Gallery&rsquo;s exclusive collection of architectural elements and
                furniture. As a proud partner of Italian luxury brand Laura Meroni, we offer a
                hand-picked selection of custom-made pieces that exceed the highest standards of
                quality and style. Each piece is meticulously crafted to elevate your space. With
                our collection, you can effortlessly add sophistication to your home or create a
                statement piece that will impress for years to come. Find your perfect match and
      ...
      <SectionBand>
        <BlueprintColumn className="pb-section">
          <VendorCta category="architectural-elements-furniture" />
          <TickRule label="All architectural elements & furniture" />
